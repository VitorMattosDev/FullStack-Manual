...
**FullStack Manual** é uma coleção de **20 manuais de referência fullstack em português do Brasil (PT-BR)**. Cada volume é um **arquivo HTML único e autocontido** — sem build, sem dependências, sem JS externo além das fontes do Google. Tema *dark warm*, syntax highlighting customizado em `<span>`, navegação por sidebar com scroll-spy e busca client-side.
...
pages/
  ...
  manual-devops.html               # Vol XIX  (Op)  ✅ 22 caps — fim da Onda 5
  manual-capstone.html             # Vol XX   (Fs)  ✅ 22 caps — Onda 6 (síntese) / coleção completa
...
| 6 Síntese | XX | Projeto Fullstack (capstone) | manual-capstone.html | Fs | ✅ |

> **Coleção completa: 20/20 volumes publicados.**
...
- Atualizar o hero: contador `N / 20 publicados` e `progress-fill` width = `N/20 * 100`%.
